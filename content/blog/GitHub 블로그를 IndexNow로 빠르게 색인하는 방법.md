...
 깃허브 블로그를 IndexNow를 통해 색인할 때 인증받는 방법에는 두 가지가 있습니다.첫 번째 방법은 **API 키를 직접적으로 루트 디렉토리에 올리지 않고, GitHub Actions를 통해 인증받는 방식**입니다. 이 방법은 GitHub Actions에서 API 키를 환경 변수로 설정하고, 이를 사용해 인증 파일을 동적으로 생성하여 보안을 유지할 수 있습니다. 예를 들어, GitHub Actions에서 `secrets.INDEXNOW_KEY` 값을 사용해 인증 파일을 생성하고, 해당 파일을 사이트 루트에 배포하는 방식입니다. 이 과정에서 API 키는 공개되지 않으며, 자동으로 색인 요청이 가능합니다.두 번째 방법은 **API 키를 직접 루트 디렉토리에 업로드한 후, 보안을 위해 기록을 지우는 방식**입니다. 이 방법에서는 API 키를 포함한 `.txt` 파일을 생성하여 사이트 루트에 업로드한 후, 검색 엔진이 소유권을 확인하면 해당 파일을 삭제할 수 있습니다. IndexNow는 처음에 소유권이 인증되면 이후에는 API 키가 없어도 색인 요청이 가능하기 때문에, 보안을 위해 파일을 삭제해도 문제가 없습니다.이러한 과정을 통해 API 키가 성공적으로 인증되면 HTTP 200 상태 코드가 반환되며, Bing의 IndexNow 페이지에서 색인이 완료된 것으로 표시됩니다. 
 
#### 방법1
1. API Key 생성하기
