...
#### 서론
Papermod 블로그의 컨텐트 파일트리를 탐색하던 중, 깃(Git)의 풀(Pull)과 푸쉬(Push)를 자동화하는 방법을 설명한 블로그를 우연히 발견했습니다. 이전에는 Obsidian을 사용하여 Git으로 자동 백업을 진행하였지만, 커밋이 과도하게 많아진다는 생각이 들었습니다. 이에 따라 Git에 익숙해진 저는 Obsidian을 시작하기 전에 `pull`하고 종료할 때 `push`하는 것이 어떨까 고민하였습니다. 그러나 이 과정이 번거로워 좋은 방법이 없을까 고민하던 중, 이 블로그를 발견하게 되었습니다.
...
#### 본론
##### 풀 명령
로컬 Git 리포지토리가 있는 경로로 이동한 후 `pull` 명령을 실행하고, 이후 Obsidian을 실행합니다.

```note-pull.ps1
Set-Location -Path D:\JONGEUN\Obsidian\Vault\
git pull
Set-Location -Path $PSScriptRoot
Start-Process -FilePath "C:\Users\gaepl\AppData\Local\Obsidian\Obsidian.exe"
```
...
이 문제에 대해 루튼에게 물어보니, PowerShell의 실행 정책을 변경하려 할 때 '관리자 권한’이 없어서 발생하는 문제라고 설명하였습니다. 이를 해결하기 위해서는 PowerShell을 '관리자 권한’으로 실행하거나, 현재 사용자에 대한 실행 정책만 변경해야 합니다.
...
2. 현재 사용자에 대한 실행 정책만 변경하기: PowerShell에서 `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser`를 실행합니다. 이 명령은 현재 사용자에 대해 인터넷에서 다운로드받은 스크립트는 디지털 서명이 있어야 실행할 수 있도록 설정하고, 로컬 컴퓨터에서 작성된 스크립트는 실행할 수 있도록 설정합니다. 이 경우, '관리자 권한’이 필요하지 않습니다. 다운로드 받은 스크립트는 서명이 필요하다는 명령을 선택해서 실행했습니다. 명령을 실행하면 예(Y), 아니오(N) 질문이 있는데 Y를 눌러 허용하였습니다. 테스트 결과 쉘이 제대로 작동하였습니다.
    

##### 간단한 명령으로 쉘 실행하기
```
Set-Alias -Name np -Value D:\JONGEUN\Obsidian\note_push.ps1
```
...
이 명령은 'np’라는 별칭을 생성하고, ‘D:\JONGEUN\Obsidian\note_push.ps1’ 파일을 가리키도록 설정합니다.
...
다음은 PowerShell 프로파일을 생성하는 방법입니다:
...
```
Set-Alias -Name npl -Value D:\JONGEUN\Obsidian\note_pull.ps1
```

이렇게 설정한 별명을 사용하면 바로 풀과 푸쉬가 실행됩니다. 이로써 Obsidian의 Git 자동 사용을 중단하게 되었습니다.
